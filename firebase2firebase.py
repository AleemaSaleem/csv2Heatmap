...
def create_heatmap(data, device_id, date, attributes, color_functions, distance_threshold=1000):
    with ThreadPoolExecutor() as executor:
        futures = [
            executor.submit(
                process_attribute, data.copy(), device_id, date, attr, color_functions[attr], distance_threshold
            )
    ...
    for device in devices:
        ...
        for date, ts_list in date_groups.items():
            ...
            if not all_data.empty:
                create_heatmap(all_data, device, date, attributes=attributes, color_functions=color_functions, distance_threshold=500)
            else:
