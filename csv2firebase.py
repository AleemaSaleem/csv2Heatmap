...
def upload_csv_to_firebase_and_store_url(csv_file, folder_name="uploads"):
    """
...
def create_heatmap(data, device_id, date, attributes, color_functions, distance_threshold=1000):
    ...
    for attribute in attributes:
        ...
        m = folium.Map(location=[points[:, 0].mean(), points[:, 1].mean()], zoom_start=17.5)
...
# devices = [ 
# "2407050002","M20-2407300002 --- "
    
#     # "M01-2401260001","M05-2309230002", "M06-2407040001"
# ]
...
# Extract unique device IDs from the CSV
devices = data['device_id'].unique()
print("Devices found in CSV:", devices)
...
def generate_heatmaps_for_devices(devices, data, attributes, color_functions):
    """
    Generate heatmaps for each device based on timestamps and data.

    Parameters:
    - devices: List of devices to process
    - data: DataFrame containing the data to process
    - attributes: List of attributes to generate heatmaps for
    - color_functions: Dictionary of color functions for each attribute
    """
...
generate_heatmaps_for_devices(devices, data, attributes, color_functions)
